...
- **From-paycheck wealth lines** — a 401k, an HSA or mortgage principal never goes through the bank account (it comes off the paycheck or rides inside another payment), so counting it against income would double-charge it. A small card icon on each wealth-building line marks it as from-paycheck: it still counts in the wealth-building total, the savings chart and the retirement predictor, but stops reducing Income Retained, the ring chart, the chips and both outlooks. One tap applies to the same-named line in every month, past and future, and the mark rides into each new month.
